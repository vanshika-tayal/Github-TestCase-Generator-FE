...
const RepositoryCard = ({ repo, onSelect }) => (
  <motion.div
    ...
    whileTap={{ scale: 0.98 }}
    className="card p-6 cursor-pointer hover:shadow-medium transition-all duration-200 h-full"
    onClick={() => onSelect(repo)}
  >
    <div className="flex items-start justify-between h-full">
      <div className="flex-1 min-w-0">
        <div className="flex items-center space-x-2 mb-2">
          <FolderGit2 className="w-5 h-5 text-primary-600 flex-shrink-0" />
          <h3 className="text-lg font-semibold text-gray-900 truncate">{repo.name}</h3>
          {repo.private && (
            <span className="badge-secondary text-xs flex-shrink-0">Private</span>
          )}
        </div>
        ...
        <div className="flex items-center space-x-4 text-sm text-gray-500 flex-wrap">
          ...
          <span className="truncate">Updated {getRelativeTime(repo.updated_at)}</span>
        </div>
      </div>
    ...
    <div className="space-y-6 w-full">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div className="text-center lg:text-left">
          ...
          <h1 className="text-3xl lg:text-4xl font-bold text-gray-900">Repository Browser</h1>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-5 gap-6">
        ...
        <div className="xl:col-span-1">
          <div className="card p-6">
            <h2 className="text-lg lg:text-xl font-semibold text-gray-900 mb-4">Repositories</h2>
            {reposLoading ? (
          ...
          <div className="card p-6">
            {!selectedRepo ? (
              <div className="text-center py-12 text-gray-500">
                ...
                <h3 className="text-lg font-semibold text-gray-900 mb-2">Select a Repository</h3>
                    ...
                    <div className="space-y-1">
                      {selectedFiles.map((file) => (
                        <div key={file.path} className="flex items-center justify-between text-sm">
                          <span className="text-primary-800">{file.name}</span>
                          <button
